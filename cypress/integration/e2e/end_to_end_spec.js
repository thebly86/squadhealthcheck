describe("End to end User journey", function() {
  ...
  it("UJ01 - Create a new Project, create a new team, sprint and fill out the health check", function() {
    ...
    cy.get(this.element.appHeaderElements.headerTitle).contains("Dashboard");

    // Create a project
    cy.get(this.element.appHeaderElements.newProjectButton).click();

    cy.get(this.element.modalElements.modalProjectNameField).type(
      newProjectName
    );

    cy.get(this.element.modalElements.modalSaveButton).click();
    ...
    cy.get(this.element.appHeaderElements.headerTitle).contains(newProjectName);
    ...
    // Save the changes
    cy.get(this.element.healthPage.healthSaveButton).click();
  ...
  after(function() {
    ...
    cy.get(this.element.modalElements.modalDeleteButton).click();
  });
});
